  ...
  */
  restore(data) {
    return new RSVP.Promise((resolve, reject) => {
      let socket = this.get('socketService').socketFor('ws://localhost:3000/');
      socket.emit('auth:authSession', {token: data.token}, (response) => {
  ...
  */
  authenticate(identification, password) {
    return new RSVP.Promise((resolve, reject) => {
      let socket = this.get('socketService').socketFor('ws://localhost:3000/');
      console.log("Session!")
      socket.emit('auth:login', {identification: identification, password: password}, (response) => {
